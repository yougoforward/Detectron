...
def add_MobileNet_conv5_body(model):
    return add_MobileNet_convX_body(model)
...
def add_MobileNet_roi_conv5_head(model, blob_in, dim_in, spatial_scale):
...
def add_block( model, prefix, blob_in,  dim_in,  dim_out, stride=2):
    """Add a ResNet stage to the model by stacking n residual blocks."""
    # e.g., prefix = res2
    # blob_in = brew.group_conv(
    #     model,
    #     blob_in,
    #     '{}_{}'.format(prefix, 'dw'),
    #     dim_in,
    ...
    blob_in = model.AffineChannel(blob_in, '{}_{}_bn'.format(prefix, 'dw'), dim=dim_in, inplace=True)
    blob_in = model.Relu(blob_in, blob_in)
    blob_in = model.Conv(blob_in, '{}_{}'.format(prefix, 'sep'), dim_in, dim_out, 1, pad=0, stride=1, no_bias=1)
    blob_in = model.AffineChannel(blob_in, '{}_{}_bn'.format(prefix, 'sep'), dim=dim_out, inplace=True)
    blob_in = model.Relu(blob_in, '{}_{}_relu'.format(prefix, 'sep'))
    dim_in = dim_out
    return blob_in, dim_in

def add_MobileNet_convX_body( model, freeze_at=2):
    """Add a ResNet body from input data up through the res5 (aka conv5) stage.
        The final res5/conv5 stage may be optionally excluded (hence convX, where
        X = 4 or 5)."""
    ...
    p = model.AffineChannel(p, 'conv1_bn', dim=32, inplace=True)
    p = model.Relu(p, 'conv1_relu')
    dim_in=32
    # block1
    s, dim_in = add_block(model, 'conv2_1', p,  dim_in,  64, stride=1)

    # block2
    s, dim_in = add_block(model, 'conv2_2', s, dim_in, 128, stride=2)

    # block3
    s, dim_in = add_block(model, 'conv3_1', s, dim_in, 128, stride=1)

    if freeze_at == 2:
        model.StopGradient(s, s)
    # block4
    s, dim_in = add_block(model, 'conv3_2', s, dim_in, 256, stride=2)

    # block5
    s, dim_in = add_block(model, 'conv4_1', s, dim_in, 256, stride=1)
    if freeze_at == 3:
        model.StopGradient(s, s)
    # block6
    s, dim_in = add_block(model, 'conv4_2', s, dim_in, 512, stride=2)

    # block7-11
    for i in xrange(7, 12):
        s, dim_in = add_block(model, 'conv5_{}'.format(i-6), s, dim_in, 512, stride=1)
    if freeze_at == 4:
        model.StopGradient(s, s)
    # block12
    s, dim_in = add_block(model, 'conv5_6', s, dim_in, 1024, stride=2)

    # block13
    s, dim_in = add_block(model, 'conv6', s, dim_in, 1024, stride=1)
    if freeze_at == 5:
        model.StopGradient(s, s)
    # Final layers
    # brew.average_pool(
