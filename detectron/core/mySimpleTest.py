...
def im_detect_all_batch(model, im, box_proposals, timers=None):
    if timers is None:
        timers = defaultdict(Timer)
    ...
    if cfg.TEST.BBOX_AUG.ENABLED:
        ...
        scores, boxes, im_scales = im_detect_bbox2(model, im, box_proposals, timers=timers)
    timers['im_detect_bbox'].toc()
...
def im_detect_all_batch_multipro(model, im, inputs, scales, timers=None):
    if timers is None:
        timers = defaultdict(Timer)

    timers['im_detect_bbox'].tic()
    # get_blob_time =time.time()
    # inputs, im_scales = _get_blobs3(im, box_proposals)
    # print("get im blob time %f.2ms"%((time.time()-get_blob_time)*1000))

    # scores, boxes, im_scales = im_detect_bbox_multipro(model, im, inputs, scales, timers=timers)
    rois, scores, box_deltas, im_scales = im_detect_bbox_multipro(model, im, inputs, scales, timers=timers)
    timers['im_detect_bbox'].toc()
    return rois, scores, box_deltas, im_scales

def im_detect_all_batch_multipro_post(im, rois, scores, box_deltas, im_scales, timers):
    scores, boxes, im_scales=im_detect_bbox_multipro_post(im, rois, scores, box_deltas, im_scales)
    # score and boxes are from the whole image after score thresholding and nms
    # (they are not separated by class)
    # cls_boxes boxes and scores are separated by class and in the format used
    # for evaluating results
...
def im_detect_bbox2(model, im, boxes=None, timers=None):
    ...
    """
    get_blob_time =time.time()
    inputs, im_scales = _get_blobs3(im, boxes)
    print("get im blob time %f.2ms"%((time.time()-get_blob_time)*1000))
    ...
    timers['batch_detect_bbox_forward'].tic()
    st1= time.time()
    for k, v in inputs.items():
        workspace.FeedBlob(core.ScopedName(k), v)
    workspace.FeedBlob(core.ScopedName('pixelmean'),cfg.PIXEL_MEANS)
    # print(workspace.Blobs())
    # data = workspace.FetchBlob('gpu_0/data')
    # conv1 = workspace.FetchBlob('gpu_0/conv1')
    # bn1_b = workspace.FetchBlob('gpu_0/res_conv1_bn_b')

    workspace.RunNet(model.net.Proto().name)

    et = time.time()-st1
    print("forward time %.2f ms"%(et*1000))
    # Read out blobs
    ...
    timers['batch_detect_bbox_forward'].toc()
    return scores, pred_boxes, im_scales

def im_detect_bbox_multipro(model, im, inputs, im_scales, timers=None):
    """Bounding box object detection for an image with given box proposals.
...
def im_detect_bbox_multipro_post(im, rois, scores, box_deltas, im_scales):
    # Read out blobs
    if cfg.MODEL.FASTER_RCNN:
        assert len(im_scales) == 1, \
            'Only single-image / single-scale batch implemented'
...
def im_detect_bbox(model, im, boxes=None):
    ...
    """
    # get_blob_time =time.time()
    inputs, im_scales = _get_blobs2(im, boxes)
    # print("get im blob time %f.2ms"%((time.time()-get_blob_time)*1000))
    # When mapping from image ROIs to feature map ROIs, there's some aliasing
    # (some distinct image ROIs get mapped to the same feature ROI).
    # Here, we identify duplicate feature ROIs, so we only compute features
...
def _get_image_blob(im):
    ...
    # et1=time.time()-st1
    # print("prep_im_for_blob time %.2f ms"%(et1*1000))
    blob = blob_utils.im_list_to_blob(processed_ims)
    # et2=time.time()-st1-et1
    # print("im_list_to_blob time %.2f ms"%(et2*1000))
...
def _get_blobs(im, rois):
    ...
    # print("getimblob time %.2fms"%((time.time()-getimblob)*1000))
    if cfg.MODEL.FASTER_RCNN and rois is None:
        height, width = blobs['data'].shape[2], blobs['data'].shape[3]
        scale = im_scale_factors[0]
        blobs['im_info'] = np.array([[height, width, scale]], dtype=np.float16)
    if rois is not None:
        blobs['rois'] = _get_rois_blob(rois, im_scale_factors)
    return blobs, im_scale_factors

def _get_blobs2(im, rois):
    """Convert an image and RoIs within that image into network inputs."""
    blobs = {}
    # getimblob=time.time()
    blobs['data'], im_scale_factors = _get_image_blob2(im)
    # print("getimblob time %.2fms"%((time.time()-getimblob)*1000))
    if cfg.MODEL.FASTER_RCNN and rois is None:
        height, width = blobs['data'].shape[2], blobs['data'].shape[3]
        scale = im_scale_factors[0]
...
def _get_blobs3(im, rois):
    """Convert an image and RoIs within that image into network inputs."""
    blobs = {}
    blobs['data'], im_scale_factors = _get_image_blob4(im)
    if cfg.MODEL.FASTER_RCNN and rois is None:
        height, width = blobs['data'].shape[2], blobs['data'].shape[3]
        scale = im_scale_factors[0]
        num_images = len(im)
        blobs['im_info'] = np.tile(np.array([height, width, scale], dtype=np.float16),(num_images,1))
        # blobs['im_info'] = np.array([[height, width, scale]], dtype=np.float16)
    if rois is not None:
        blobs['rois'] = _get_rois_blob(rois, im_scale_factors)
    # blobs['img'] = im[0]
...
def _get_image_blob3(images):
    ...
    Returns:
        ...
        im_scale_factors (ndarray): array of image scales (relative to im) used
            in the image pyramid
    """
    images = [im.astype(np.float32, copy=False) for im in images]
    ...
    for target_size in cfg.TEST.SCALES:
        ...
        ims = [cv2.resize(im, None, None, fx=im_scale, fy=im_scale,
...
def _get_image_blob2(im):
    ...
    ims = []
    im_scales = []
    #prep_im_for_blob_time = time.time()
    for target_size in cfg.TEST.SCALES:
        im_scale = float(target_size) / float(im_size_min)
        # Prevent the biggest axis from being more than max_size
        if np.round(im_scale * im_size_max) > cfg.TEST.MAX_SIZE:
        ...
        # # Cr = imgYcc[:,:,1]
        # # Cb = imgYcc[:,:,2]
        # clahe = cv2.createCLAHE(clipLimit=20.0, tileGridSize=(36, 36))
        # imY = clahe.apply(Y)
        # imgYcc[:, :, 0]=imY
        # im = cv2.cvtColor(imgYcc, cv2.COLOR_YCR_CB2BGR)
        # # cv2.imwrite('clahe_2.jpg', cl1)
        # im = im.astype(np.float32, copy=False)
        im -= cfg.PIXEL_MEANS
        #t2 = time.time() - reducemean_time
        #print("image reduce mean time %.2fms" % (t2 * 1000))
        ims.append(im)
        im_scales.append(im_scale)
    ...
    #print("im_list_to_blob time %.2f ms"%(et2*1000))
    return blob, im_scales
    # np.array(im_scale_factors, dtype=np.float16)

def _get_image_blob4(im):
    """Converts an image into a network input.
    ...
    Returns:
        ...
        im_scale_factors (ndarray): array of image scales (relative to im) used
            in the image pyramid
    """
    ori_ims = [i.astype(np.float32, copy=False) for i in im]
    im_shape = ori_ims[0].shape
    im_size_min = np.min(im_shape[0:2])
    im_size_max = np.max(im_shape[0:2])
    ims=[]
    im_scales = []
