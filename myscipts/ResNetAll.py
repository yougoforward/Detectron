...
def add_ResNet34_conv5_body(model):
    return add_ResNet18_convX_body(model, (3, 4, 6, 3))
...
def add_ResNet18_convX_body(model, block_counts, freeze_at=2):
    ...
    # data = model.net.FloatToHalf('data', 'data' + '_fp16')
    #data = model.net.Sub(['data', 'pixelmean'],'reduceMean', broadcast=1,axis=1)
    #data = model.net.FloatToHalf(data, 'data' + '_fp16')
    ...
    s, dim_in = add_stage_basic(model, 'res2', p, n1, dim_in, 64, 1)
    if freeze_at == 2:
        model.StopGradient(s, s)
    s, dim_in = add_stage_basic(
        model, 'res3', s, n2, dim_in, 128, 1
    )
    if freeze_at == 3:
        model.StopGradient(s, s)
    s, dim_in = add_stage_basic(
        model, 'res4', s, n3, dim_in, 256, 1
    )
    if freeze_at == 4:
        model.StopGradient(s, s)
...
def add_stage_basic(model,
    ...
    # e.g., prefix = res2
    for i in range(n):
        blob_in = add_residual_block_basic(
            model,
            '{}_{}'.format(prefix, i),
            blob_in,
            dim_in,
...
def basicblock_transformation(
    ...
    # (stride, str3x3) = (stride, 1) if cfg.RESNETS.STRIDE_1X1 else (1, stride)
...
def add_ResNet_convX_body(model, block_counts, freeze_at=2):
    ...
    assert freeze_at in [0, 2, 3, 4, 5]
    p = model.Conv('data', 'conv1', 3, 64, 7, pad=3, stride=2, no_bias=1)
    # p = model.HalfToFloat(p, p)
    # p = model.AffineChannel(p, 'res_conv1_bn', dim=64, inplace=True)
    p = model.SpatialBN(p, 'res_conv1_bn', dim_in=64, is_test=False, ScaleInitializer = pFP16Initializer, BiasInitializer = pFP16Initializer,
    RunningMeanInitializer = pFP16Initializer, RunningVarianceInitializer = pFP16Initializer)
    # p = model.FloatToHalf(p, p)
    p = model.Relu(p, p)
    p = model.MaxPool(p, 'pool1', kernel=3, pad=1, stride=2)
    dim_in = 64
    dim_bottleneck = cfg.RESNETS.NUM_GROUPS * cfg.RESNETS.WIDTH_PER_GROUP
